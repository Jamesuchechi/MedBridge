...
.crx-title { font-family:'Syne',sans-serif; font-size:clamp(22px,3vw,32px); font-weight:800; margin-bottom:6px; line-height:1.1; color:var(--text); }
...
.crx-sub { font-size:14px; color:var(--text2); margin-bottom:24px; line-height:1.6; }
...
.crx-input { width:100%; padding:13px 13px 13px 40px; background:var(--bg2); border:1.5px solid var(--border); border-radius:14px; color:var(--text); font-size:14px; outline:none; transition:all .2s; }
.crx-input:focus { border-color:rgba(0,229,160,.5); box-shadow:0 0 0 3px rgba(0,229,160,.08); }

.crx-select { padding:0 16px; height:50px; background:var(--bg2); border:1.5px solid var(--border); border-radius:14px; color:var(--text); font-size:14px; outline:none; cursor:pointer; min-width:140px; transition:all .2s; }
.crx-select:focus { border-color:var(--accent); }
.crx-select option { background:var(--bg3); color:var(--text); }
...
.crx-banner svg { width:16px; height:16px; flex-shrink:0; margin-top:1px; color:var(--accent2); }

/* ── Grid ── */
.crx-grid { display:grid; grid-template-columns:repeat(auto-fill,minmax(290px,1fr)); gap:14px; }
.crx-card { background:var(--card-bg); border:1px solid var(--card-border); border-radius:18px; padding:20px; cursor:pointer; transition:all .22s; position:relative; }
.crx-card:hover { border-color:rgba(0,229,160,.35); transform:translateY(-2px); box-shadow:0 10px 32px rgba(0,0,0,.15); }
...
.crx-card-name { font-family:'Syne',sans-serif; font-size:15px; font-weight:700; margin-bottom:6px; padding-right:50px; color:var(--text); }
...
.crx-trust-bar { width:40px; height:4px; border-radius:2px; background:var(--border); overflow:hidden; }
...
.crx-card-phone { font-size:11px; color:var(--accent2); margin-top:6px; }
.crx-directions-link { font-size:11px; color:var(--accent2); font-weight:700; display:flex;align-items:center;gap:3px; text-decoration:none; margin-left:auto; }
...
.crx-dn { font-family:'Syne',sans-serif; font-size:22px; font-weight:800; color:var(--text); }
...
.crx-btn-outline-blue  { background:rgba(61,155,255,.08); border:1.5px solid rgba(61,155,255,.25); color:var(--accent2); }
.crx-action-btn:hover { transform:translateY(-1px); }

.crx-section { background:var(--card-bg); border:1px solid var(--card-border); border-radius:18px; padding:20px; margin-bottom:16px; }
...
.crx-avail-row { display:flex; align-items:center; justify-content:space-between; padding:10px 0; border-bottom:1px solid var(--border); }
.crx-avail-row:last-child { border-bottom:none; }
.crx-avail-name { font-size:14px; font-weight:600; color:var(--text); }
...
.crx-badge-out  { background:rgba(255,92,92,.1); color:var(--danger); border:1px solid rgba(255,92,92,.2); }
.crx-price-row { display:flex; align-items:center; justify-content:space-between; padding:10px 0; border-bottom:1px solid var(--border); }
.crx-price-row:last-child { border-bottom:none; }
.crx-price-drug { font-size:14px; font-weight:600; color:var(--text); }
...
/* ── Report modal ── */
.crx-overlay { position:fixed; inset:0; background:rgba(0,0,0,.5); backdrop-filter:blur(8px); z-index:9999; display:flex;align-items:center;justify-content:center; padding:20px; animation:crx-fade .2s ease; }
@keyframes crx-fade { from{opacity:0} to{opacity:1} }
.crx-modal { width:100%; max-width:460px; background:var(--bg3); border:1px solid var(--border); border-radius:24px; padding:32px; position:relative; box-shadow:0 30px 60px rgba(0,0,0,.3); }
.crx-modal-close { position:absolute; top:18px; right:18px; width:32px; height:32px; border-radius:50%; background:var(--glass); border:none; color:var(--text2); display:flex;align-items:center;justify-content:center; cursor:pointer; }
.crx-modal-close svg { width:16px; height:16px; }
.crx-modal h3 { font-family:'Syne',sans-serif; font-size:18px; font-weight:800; margin-bottom:4px; color:var(--text); }
...
.crx-field-input { width:100%; padding:10px 14px; background:var(--bg2); border:1.5px solid var(--border); border-radius:11px; color:var(--text); font-size:14px; outline:none; transition:all .2s; }
.crx-field-input:focus { border-color:rgba(0,229,160,.5); }
.crx-toggle-row { display:flex; gap:8px; }
.crx-toggle { flex:1; padding:10px; border:1.5px solid var(--border); border-radius:11px; background:var(--bg2); color:var(--text2); font-size:13px; font-weight:700; cursor:pointer; transition:all .2s; display:flex;align-items:center;justify-content:center;gap:6px; }
...
.crx-toggle.no  { border-color:rgba(255,92,92,.5); background:rgba(255,92,92,.1); color:var(--danger); }
...
.crx-spinner { width:32px; height:32px; border:3px solid var(--border); border-top-color:var(--accent); border-radius:50%; animation:spin .7s linear infinite; }
      ...
      <div className="crx-modal" onClick={(e) => e.stopPropagation()}>
        ...
        {done ? (
          <div style={{ textAlign: "center", padding: "16px 0" }}>
            ...
            <p style={{ color: "rgba(255,255,255,0.6)", fontSize: 13 }}>Your report helps the whole community find medicines.</p>
          </div>
        ) : (
          <>
...
function PharmacyDetailView({
  ...
  return (
    <div className="crx-detail">
      <button className="crx-back" onClick={onBack}><Ic.Back /> Back to search</button>
      ...
      <div className="crx-dh">
        ...
        <div style={{ flex: 1 }}>
          ...
          {pharmacy.state && (
            <div style={{ fontSize: 12, color: "rgba(255,255,255,0.4)", marginTop: 2 }}>{pharmacy.state}</div>
          )}
        ...
        {detail && (
          <div className="crx-trust" style={{ flexDirection: "column", alignItems: "flex-end", gap: 3 }}>
            <div style={{ fontSize: 9, color: "rgba(255,255,255,0.4)", fontFamily: "DM Mono" }}>TRUST SCORE</div>
            <div className="crx-trust-bar" style={{ width: 60 }}>
              <div className="crx-trust-fill" style={{ width: `${detail.trustScore || 50}%` }} />
            </div>
            <div style={{ fontSize: 11, fontFamily: "DM Mono", color: "rgba(255,255,255,0.6)" }}>
...
export default function CommunityPage() {
  ...
  const doSearch = useCallback(async (q: string, state?: string, lat?: number, lng?: number) => {
    if (!q.trim() && !state && (lat == null || lng == null)) {
      setResults([]);
      return;
    }
    setLoading(true);
  ...
  useEffect(() => {
    ...
    if (!query.trim()) {
      if (!selectedState && !coords) setResults([]);
      return;
    }
    searchTimer.current = setTimeout(() => doSearch(query, selectedState, coords?.lat, coords?.lng), 500);
  ...
  const handleLocate = () => {
    ...
    navigator.geolocation.getCurrentPosition(
      ...
      () => {
        setLocating(false);
        toast.error("Could not get your location. Trying default search...");
  ...
  return (
    <>
      ...
      <div className="crx">
        {/* Header */}
        <div className="crx-eyebrow"><div className="crx-eyebrow-dot" /> Community Hub</div>
        <h1 className="crx-title">Find <span>Pharmacies</span> Near You</h1>
        <p className="crx-sub">
          Discover nearby pharmacies, check drug availability in real-time, and compare community-reported prices. 
          Help your community by reporting stock and price updates.
        </p>
        ...
        <div className="crx-search-bar">
          <div className="crx-input-wrap">
            ...
            <input
              className="crx-input"
              placeholder="Search by area, LGA, or name — e.g. 'Ikeja', 'HealthPlus'"
              value={query}
              onChange={(e) => setQuery(e.target.value)}
              onKeyDown={(e) => e.key === 'Enter' && doSearch(query, selectedState, coords?.lat, coords?.lng)}
              autoFocus
            />
          </div>
          <select
        ...
        <div className="crx-banner">
          <Ic.Info />
          <span>Select a pharmacy to view detailed stock reports and contribute your own community data.</span>
        </div>
        ...
        {!loading && results.length > 0 && (
          <>
            <div style={{ fontSize: 12, color: "rgba(255,255,255,0.4)", fontFamily: "DM Mono, monospace", marginBottom: 14 }}>
                ...
                return (
                  <div key={key} className="crx-card" onClick={() => setSelected(p)}>
                    ...
                    <div className="crx-card-meta">
                      ...
                      <div className="crx-directions-link">
                        Details <Ic.Ext />
                      </div>
                    </div>
                  </div>
        ...
        {!loading && !results.length && (
          <div className="crx-empty">
            ...
            <div style={{ fontSize: 16, fontWeight: 700, marginBottom: 8, color: 'var(--text)' }}>Start searching local pharmacies</div>
            <div style={{ maxWidth: 400, margin: "0 auto", fontSize: 14, color: 'var(--text2)' }}>
              Search by name, select a state, or tap <strong>Use my location</strong> to discover pharmacies in your community.
            </div>
          </div>
        )}
      </div>
